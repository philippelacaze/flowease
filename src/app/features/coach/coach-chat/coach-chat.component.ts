...
export class CoachChatComponent implements OnInit, AfterViewChecked {
  ...
  protected async onNewConversation(): Promise<void> {
    if (this.sessionId) {
      await this.coach.summarizeSession(this.sessionId);
    }
    this.messages = [];
    this.sessionId = null;
  ...
  protected async onSendMessage(text?: string): Promise<void> {
    ...
    try {
      for await (const token of this.coach.sendMessage(input)) {
  ...
  private async initSession(contextWindow: CoachContextWindow): Promise<void> {
    const [result, context] = await Promise.all([
      this.coach.startSession(contextWindow),
      this.coach.buildContext(contextWindow),
    ]);
  ...
  private async applyPickerResult(result: StartCoachSessionResult): Promise<void> {
    const context = await this.coach.buildContext(result.contextWindow);
    this.sessionId = result.sessionId;
    this.previousSummary = result.previousSummary;
    this.coachContext = context;
