...
const mockCoach = {
  ...
  buildContext: vi.fn().mockResolvedValue(DEFAULT_CONTEXT),
  startSession: vi.fn().mockResolvedValue(DEFAULT_SESSION),
};
...
describe('CoachChatComponent', () => {
  ...
  describe('initialisation automatique', () => {
    it('démarre la session avec le contexte par défaut sans ouvrir le bottom sheet', async () => {
      ...
      expect(mockCoach.startSession).toHaveBeenCalledWith('14d');
      expect(mockBottomSheet.open).not.toHaveBeenCalled();
    });

    it('construit le contexte pour la fenêtre par défaut', async () => {
      await createComponent();
      expect(mockCoach.buildContext).toHaveBeenCalledWith('14d');
    });
        ...
        providers: [
          provideRouter([]),
          {
            ...
            useValue: {
              sendMessage: vi.fn().mockReturnValue((async function* () {})()),
              summarizeSession: vi.fn().mockResolvedValue(undefined),
              buildContext: vi.fn().mockReturnValue(new Promise(() => {})),
  ...
  describe('nouvelle conversation', () => {
    it('démarre une nouvelle session avec le contexte par défaut sans ouvrir le picker', async () => {
      const fixture = await createComponent();
      mockCoach.startSession.mockClear();
