...
const mockStartSession = {
  execute: vi.fn().mockResolvedValue({ sessionId: 's1', contextWindow: '7d', previousSummary: undefined }),
};
...
describe('CoachContextPickerComponent', () => {
  ...
  describe('sélection d\'une option', () => {
    it('appelle StartCoachSessionUseCase avec la clé sélectionnée', async () => {
      ...
      option7d.click();
      await fixture.whenStable();
      expect(mockStartSession.execute).toHaveBeenCalledWith('7d');
    });
    ...
    it('n\'appelle pas execute une deuxième fois si loading est en cours', async () => {
      let resolve!: () => void;
      mockStartSession.execute.mockReturnValue(new Promise<void>(r => { resolve = r; }));
      const fixture = await createComponent('14d');
